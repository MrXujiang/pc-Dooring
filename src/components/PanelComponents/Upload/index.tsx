...
class PicturesWall extends React.Component<PicturesWallType> {
  ...
  render() {
    const { previewVisible, previewImage, fileList, previewTitle, wallModalVisible, imgBed, curSelectedImg } = this.state;
    const {
      action = isDev ? 'http://192.168.1.3:3000/api/v0/files/upload/free' : 'http://h5.dooring.cn/api/v0/files/upload/free',
      headers,
    ...
    return <>
      {
        isCrop ? (
          <ImgCrop modalTitle="裁剪图片" modalOk="确定" modalCancel="取消" rotate={true} aspect={cropRate}>
            <Upload
              fileList={fileList}
      ...
      <div className={styles.wallBtn} onClick={this.handleWallShow}>图片库</div>
      <Modal
      ...
      >
        <Tabs defaultActiveKey={cates[0]} tabPosition="left" style={{ height: 520 }}>
          {
            cates.map((item, i) => {
              return <TabPane tab={wallCateName[item]} key={item}>
                      <div className={styles.imgBox}>
                        {
                          (imgBed as any)[item] && (imgBed as any)[item].map((item:string, i:number) => {
                            return <div className={classnames(styles.imgItem, curSelectedImg === item ? styles.seleted : '')} key={i} onClick={() => this.handleImgSelected(item)}>
                                    <img src={item} alt="趣谈前端-h5-dooring"/>
                                    <span className={styles.iconBtn}><CheckCircleFilled /></span>
                                  </div>
                          })
                        }
                      </div>
